...
export default function Configuracion(){
  const [data,setData]=useState(null),[defaults,setDefaults]=useState(null),[message,setMessage]=useState(''),[messageType,setMessageType]=useState('info'),[busy,setBusy]=useState(false);
  const [password,setPassword]=useState({actual:'',nueva:'',confirmacion:''});
  useEffect(()=>{api('/admin/configuracion').then(r=>{setData(r.datos);setDefaults(r.defaults);}).catch(e=>{setMessageType('error');setMessage(e.message);});},[]);
  const change=(k,v)=>setData(d=>({...d,[k]:v}));
  async function save(e){e.preventDefault();setBusy(true);try{const r=await api('/admin/configuracion',{method:'PUT',body:data});setData(r.datos);setMessageType('success');setMessage(r.message);}catch(err){setMessageType('error');setMessage(err.message);}finally{setBusy(false);}}
  async function changePassword(e){e.preventDefault();if(password.nueva!==password.confirmacion){setMessageType('error');setMessage('Las contraseñas no coinciden.');return;}setBusy(true);try{await api('/admin/password',{method:'PUT',body:password});logout();window.location.assign('/login');}catch(err){setMessageType('error');setMessage(err.message);}finally{setBusy(false);}}
